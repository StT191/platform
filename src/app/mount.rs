...
impl<App: AppHandler> AppMount<App> {
  ...
  pub fn with_window(mut event_queue: Vec<AppEvent>, event_loop_proxy: AppEventLoopProxy, window: Window, init_data: App::InitData) -> Self {
    event_queue.push(AppEvent::Resumed);
    Self { state: MountState::Window { event_queue, event_loop_proxy, window, init_data } }
  }
